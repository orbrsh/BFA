...
/**
 * An booksForAll listener that reads the purchase json file and populates the data into a Derby database
 */
@WebListener
public class ManagePurchaseDB implements ServletContextListener {

    /**
     * Default constructor. 
     */
    public ManagePurchaseDB() {
        // TODO Auto-generated constructor stub
    }
    
    //utility that checks whether the purchase tables already exists
    	...
    	try{
    		...
    		if (!created){
    			...
    			for (Purchase purchase : purchases){
    				pstmt.setLong(1,purchase.getIdParchased());
    				pstmt.setLong(2,purchase.getIdBook());
    				pstmt.setString(3,purchase.getUsername());
    				pstmt.setTimestamp(4,purchase.getDataBought());
    				pstmt.executeUpdate();
    			}
    ...
    public void contextDestroyed(ServletContextEvent event)  { 
         ...
         //shut down database
    	 try {
     		//obtain LikeDB data source from Tomcat's context and shutdown
     		Context context = new InitialContext();
     		BasicDataSource ds = (BasicDataSource)context.lookup(
